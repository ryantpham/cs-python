...
def downloadCameraFileToTrace(camera, remote_file_path, remote_file_name, trace_path):
    try:
        ...
        client.connect(camera.ip, username='root', password='gku88888')

        # Construct and execute the download command
        command = f'python camera_download_file.py {remote_file_path} {remote_file_name} {trace_path}'
        ...
        if exit_status == 0:
            print(f"Successfully downloaded {remote_file_name} from {camera.ip}")
            return True
        else:
            print(f"Failed to download {remote_file_name} from {camera.ip}")
            return False
